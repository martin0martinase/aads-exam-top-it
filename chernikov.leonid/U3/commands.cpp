...
void chernikov::initAppState(AppState &state)
{
  ...
  initMeetsByPerson(state.meets);
  initDateRange(state.currentRange);
  initRangeStack(state.rangeStack);
  initHashTable(state.validDates);
}
...
void chernikov::destroyAppState(AppState &state)
{
  ...
  destroyMeetsByPerson(state.meets);
  destroyRangeStack(state.rangeStack);
  destroyHashTable(state.validDates);
}
...
void chernikov::loadMeetsFromFile(AppState &state, const std::string &filename)
{
  ...
  while (std::getline(file, line))
  {
    ...
    if (state.currentRange.isEmpty)
    {
      state.currentRange.minDate = currentDate;
      state.currentRange.maxDate = currentDate;
      state.currentRange.isEmpty = false;
    }
    else
    {
      if (dateLess(currentDate, state.currentRange.minDate))
      {
        state.currentRange.minDate = currentDate;
      }
      if (dateGreater(currentDate, state.currentRange.maxDate))
      {
        state.currentRange.maxDate = currentDate;
      }
    }

    addDate(state.validDates, currentDate);
...
void chernikov::executeCommand(AppState &state, const std::string &line)
{
  ...
  if (command == "meets")
  {
    ...
    MeetList *list = getMeetsByPerson(state.meets, id);
  ...
  if (command == "commons")
  {
    ...
    MeetList *list1 = getMeetsByPerson(state.meets, id1);
    MeetList *list2 = getMeetsByPerson(state.meets, id2);
  ...
  if (command == "range")
  {
    ...
    {
      printDate(state.currentRange.minDate, std::cout);
      std::cout << " : ";
      printDate(state.currentRange.maxDate, std::cout);
      std::cout << "\n";
    }
    return;
  }
  ...
  if (command == "after")
  {
    ...
    Date afterDate{day, month, year};

    if (dateGreaterOrEqual(afterDate, state.currentRange.maxDate))
    {
      printInvalidCommand();
      return;
    }

    pushRange(state.rangeStack, state.currentRange);

    if (dateGreater(afterDate, state.currentRange.minDate))
    {
      state.currentRange.minDate = afterDate;
    }
  ...
  if (command == "before")
  {
    ...
    Date beforeDate{day, month, year};

    if (dateLessOrEqual(beforeDate, state.currentRange.minDate))
    {
      printInvalidCommand();
      return;
    }
    ...
    if (dateLess(beforeDate, state.currentRange.maxDate))
    {
      state.currentRange.maxDate = beforeDate;
    }
  ...
  {
    DateRange prevRange;
    if (!popRange(state.rangeStack, prevRange))
    {
      printInvalidCommand();
      return;
    }
    state.currentRange = prevRange;
    return;
  }
